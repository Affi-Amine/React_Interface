...
const AddAppointment = ({onSendAppointment, lastId}) => {
  ...
  function formDataPublish() {
    ...
    return (
      <div>
        <button onClick={ () => { setToggle(!toggleForm);}}
        //template literal using the ` backtick
        className={`bg-blue-400 text-white px-2 py-3 w-full text-left rounded-t-md
        ${toggleForm ? 'rounded-md-t' : 'rounded-md'}`}>
        ...
        </button>
        {
          ...
          <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start  sm:pt-5">
            ...
            <div className="mt-1 sm:mt-0 sm:col-span-2">
              ...
              //we didn't write event => {event.target.value} beucause it will delete all the other inputs
